$(document).ready(function () {
    ...
    $(document).on('click', '.addtocart-btn', function (e) {
        ...
        var productPrice = toPositiveFloat(productPriceRaw);
...
function recalculateCart(){
    ...
    $('.product-position').each(function () {
        ...
        var productPrice = toPositiveFloat($(this).find('.price span').text());
...
function toPositiveFloat(rawVal) {
    return parseFloat((Math.round(rawVal * 100) / 100).toFixed(2));
}
...
function addToNavbarCart(quantity, price){
    ...
    var oldSum = toPositiveFloat(sumSelector.text());
    ...
    if (quantity > 0 && price > 0) {
        ...
        sumSelector.text(toPositiveFloat(newSum));
    }
}
